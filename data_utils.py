...
def read_inputs_1M(images_file, labels_file, data_dir):
    """
    Reads the images and labels from files
    Input: 
        images_file: the file containing image filenames
    ...
    """
    images = []
    with open("{}/{}".format(data_dir, images_file)) as f:
        for line in f:
            images.append(line.split()[0])
...
def get_ingredients_list_1M(data_dir):
    '''
    Returns the list of ingredients. 
    '''
    filename = data_dir + "/IngredientList.txt"
    ingredients = []
    with open(filename) as f:
        for line in f:
            ingredients.append(line.rstrip("\n"))
